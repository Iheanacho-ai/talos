...
Here is the Talos PSA plugin configuration:
...
* Throws an error, if the `restricted` profile is violated, but does not enforce this profile.
...
This Deployment complies with the `restricted` profile and does not throw any errors when applied:
...
* When you describe the DaemonSet, you can see that the Pod Security Admission plugin errors is blocking pod creation:
